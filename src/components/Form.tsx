      ...
      {error ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-2 gap-4 font-light text-white text-sm">
            ...
            <div>
              <input className="w-full bg-primary h-11 text-white px-4" placeholder={dataContact[lan].city} {...register("city")} />
            </div>
            <div>
              <input className="w-full bg-primary h-11 text-white px-4" placeholder={dataContact[lan].email} {...register("email", { required: true })} />
              {errors.email && <Error />}
            </div>
            <div>
              <input className="w-full bg-primary h-11 text-white px-4" placeholder={dataContact[lan].location} {...register("location")} />
            </div>
            <div>
              <input className="w-full bg-primary h-11 text-white px-4" placeholder={dataContact[lan].phone} {...register("phone", { required: true })} />
              {errors.phone && <Error />}
            </div>
            <div>
              <input className="w-full bg-primary h-11 text-white px-4" placeholder={dataContact[lan].country} {...register("country")} />
            </div>
          </div>
          <textarea className="w-full bg-primary h-44 text-white p-4 my-4 font-light" placeholder={dataContact[lan].message} {...register("message")} />
